...
- Revisit startup context only as a tiny canonical-keys card with selected pinned/keyed memories after retrieval evals show search/fallback is insufficient; keep it hard-capped, opt-in or narrowly configured, measured by the token-injection gate, and within the boundary set by [ADR 008](docs/adr/008-turn-start-context-boundary.md).
