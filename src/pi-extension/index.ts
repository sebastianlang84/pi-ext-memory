...
export default function registerPiMemoryExtension(pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    ctx.ui.setStatus(
      "pi-memory",
      "pi-memory v1.0.1 ready — retrieval hook, /memory-status, /memory-search, /memory-review, /memory-session-save, memory_search, memory_save, memory_update, memory_link, memory_archive",
    );
  });
