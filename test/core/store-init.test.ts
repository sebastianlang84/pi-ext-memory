...
test("initializeMemoryStore creates a fresh database and applies the latest schema", () => {
  ...
  assert.equal(store.latestSchemaVersion, 10);
  ...
  try {
    ...
    assert.equal(schemaVersion.user_version, 10);
...
test("initializeMemoryStore is idempotent for an already-migrated database", () => {
  ...
  try {
    assert.equal(secondStore.schemaVersion, 10);
    assert.equal(secondStore.latestSchemaVersion, 10);
